...
func (req *softwarePkgRequest) toCmd(user *domain.User, ua useradapter.UserAdapter) (
	...
	cmd.Spec, err = dp.NewSpecFile(req.Spec)
	if err != nil {
		return
	}

	cmd.SRPM, err = dp.NewSRPMFile(req.SRPM)
	if err != nil {
		return
	}
...
func (req *reqToUpdateSoftwarePkg) toCmd(
	...
	if req.Spec != "" {
		if cmd.Spec, err = dp.NewSpecFile(req.Spec); err != nil {
			return
		}
	}

	if req.SRPM != "" {
		if cmd.SRPM, err = dp.NewSRPMFile(req.SRPM); err != nil {
			return
		}
	}
